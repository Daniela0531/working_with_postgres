			 ...
			 QueryCompletion *qc)
{
	 ...
	 */
	elog(LOG, "my log in ProcessQuery next ExecutorStart");
	...
	PG_TRY();
	{
		...
		switch (portal->strategy)
		{
			case PORTAL_ONE_SELECT:
				 ...
				 * Call ExecutorStart to prepare the plan for execution
				 */
				elog(LOG, "my log in PortalStart next ExecutorStart");
		...
		switch (portal->strategy)
		{
			...
			case PORTAL_UTIL_SELECT:
				...
				if (portal->strategy != PORTAL_ONE_SELECT && !portal->holdStore) {
					// elog(WARNING, "my log in PortalRun next FillPortalStore");
			...
			case PORTAL_MULTI_QUERY:
				// elog(WARNING, "my log in in PortalRun next PortalRunMulti");
...
FillPortalStore(Portal portal, bool isTopLevel)
{
	...
	switch (portal->strategy)
	{
		...
		case PORTAL_ONE_MOD_WITH:
			 ...
			 */
			// elog(WARNING, "my log in in FillPortalStore next PortalRunMulti");
			PortalRunMulti(portal, isTopLevel, true,
						   treceiver, None_Receiver, &qc);
			break;

		case PORTAL_UTIL_SELECT:
			// elog(WARNING, "my log in FillPortalStore next PortalRunUtility");
...
PortalRunUtility(Portal portal, PlannedStmt *pstmt,
				 ...
				 DestReceiver *dest, QueryCompletion *qc)
{
	// elog(WARNING, "my log in PortalRunUtility");
	...
	foreach(stmtlist_item, portal->stmts)
	{
		...
		if (pstmt->utilityStmt == NULL)
		{
			...
			if (pstmt->canSetTag)
			{
				...
				// elog(WARNING, "my log in PortalRunMulti next ProcessQuery");
				ProcessQuery(pstmt,
							 portal->sourceText,
							 portal->portalParams,
							 portal->queryEnv,
			...
			else
			{
				...
				// elog(WARNING, "my log in PortalRunMulti next ProcessQuery else");
				ProcessQuery(pstmt,
							 portal->sourceText,
							 portal->portalParams,
							 portal->queryEnv,
		...
		{
			// elog(WARNING, "my log in PortalRunMulti in else");
			/*
			...
			if (pstmt->canSetTag)
			{
				...
				/* statement can set tag string */
				// elog(WARNING, "my log in PortalRunMulti next PortalRunUtility");
	...
	PG_TRY();
	{
		...
		switch (portal->strategy)
		{
			...
			case PORTAL_UTIL_SELECT:
				 ...
				 */
				if (!portal->holdStore) {
					// elog(WARNING, "my log in PortalRunFetch next FillPortalStore");
