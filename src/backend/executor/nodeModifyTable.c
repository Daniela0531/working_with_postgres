		   ...
		   ResultRelInfo **insert_destrel)
{
	...
	vlock:
		...
		else
		{
			...
			elog(LOG, "my log in ExecInsert next table_tuple_insert");
						 ...
						 ResultRelInfo **insert_destrel)
{
	...
	*retry_slot = NULL;
	...
	elog(LOG, "my log in ExecCrossPartitionUpdate next ExecInsert");
...
lreplace:
	...
	if (partition_constraint_failed)
	{
		 ...
		 */
		elog(LOG, "my log in ExecUpdateAct next ExecCrossPartitionUpdate");
		   ...
		   TupleTableSlot *oldslot, bool canSetTag, bool locked)
{
	...
	else
	{
		 ...
		 */
redo_act:
		elog(LOG, "my log in ExecUpdate next ExecUpdateAct");
	...
	foreach(l, actionStates)
	{
		...
		switch (commandType)
		{
			case CMD_UPDATE:
				...
				{
					elog(LOG, "my log in ExecMergeMatched next ExecUpdateAct");
	...
	foreach(l, actionStates)
	{
		...
		switch (commandType)
		{
			case CMD_INSERT:
				...
				elog(LOG, "my log in ExecMergeNotMatched next ExecInsert");
	...
	for (;;)
	{
		...
		switch (operation)
		{
			case CMD_INSERT:
				...
				elog(LOG, "my log in ExecModifyTable next ExecInsert");
...
ExecInitModifyTable(ModifyTable *node, EState *estate, int eflags)
{
	...
	elog(LOG, "my log in ExecInitModifyTable next ExecModifyTable");
