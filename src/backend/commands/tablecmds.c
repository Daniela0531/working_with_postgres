...
// char*
// get_tablespace_compression(Oid tablespaceId)
// {
//     HeapTuple tp;
//     Form_pg_tablespace spctuple;
    
//     tp = SearchSysCache1(TABLESPACEOID, ObjectIdGetDatum(tablespaceId));
//     if (!HeapTupleIsValid(tp))
//         return NULL;

//     spctuple = (Form_pg_tablespace) GETSTRUCT(tp);

//     // Fetch compression type; assuming spcoptions holds the compression info
//     Datum spcoptions = spctuple->spcoptions;
//     char *compression = TextDatumGetCString(spcoptions);
// 	printf("\n spcoptions : %s\n", compression);
// 	fflush(stdout);
//     ReleaseSysCache(tp);
			   ...
			   ObjectAddress *typaddress, const char *queryString)
{
	...
	// my log my realisation
	if (reloptions) {
		Datum* elements_rel;
		bool* nulls_rel;
		int nelems_rel;

		ArrayType* array_reloptions = DatumGetArrayTypeP(reloptions);

		deconstruct_array(array_reloptions, TEXTOID, -1, false, 'i', &elements_rel, &nulls_rel, &nelems_rel);

		char *tmp_rel = TextDatumGetCString(elements_rel[0]);
		elog(WARNING,"\n my log reloptions : %s\n", tmp_rel);
	...
	if (HeapTupleIsValid(tp))
	{
		// elog(WARNING, "\n my log tablespace searchsyscache success\n");
		// fflush(stdout);

		bool is_null;

		// elog(WARNING,"\n my log tablespace get tuple success\n");
		// fflush(stdout);

			/* Must get indclass the hard way */
		Datum spcoptions_dat = SysCacheGetAttr(TABLESPACEOID, tp,
											Anum_pg_tablespace_spcoptions, &is_null);

		Datum* elements;
		bool* nulls;
		int nelems;

		ArrayType* array = DatumGetArrayTypeP(spcoptions_dat);

		deconstruct_array(array, TEXTOID, -1, false, 'i', &elements, &nulls, &nelems);

		for (int i = 0; i < nelems; ++i) {
			char *tmp = TextDatumGetCString(elements[i]);
			if (strcmp("compression=brotli", tmp) == 0) {
				compression = "brotli";
				break;
			} else if (strcmp("compression=pglz", tmp) == 0) {
				compression = "pglz";
				break;
			} else if (strcmp("compression=lz4", tmp) == 0) {
				compression = "lz4";
				break;
			}
		}
	...
	// elog(WARNING, "my log compression define...\n");
	if (compression)
	{
		ListCell   *l;
	...
	// elog(WARNING, "my log compression defined\n");
	 ...
	 * below.
	 */
	// elog(WARNING, "my log in DefineRelation next BuildDescForRelation");
...
BuildDescForRelation(const List *columns)
{
	...
	foreach(l, columns)
	{
		...
		// elog(WARNING, "my log in BuildDescForRelation next GetAttributeCompression");
		   ...
		   AlterTableUtilityContext *context)
{
	...
	// elog(WARNING, "my log in AlterTable next ATController");
...
AlterTableInternal(Oid relid, List *cmds, bool recurse)
{
	...
	// elog(WARNING, "my log in AlterTableInternal next ATController");
			 ...
			 AlterTableUtilityContext *context)
{
	...
	// elog(WARNING, "my log in ATController next ATRewriteCatalogs");
				  ...
				  AlterTableUtilityContext *context)
{
	...
	for (AlterTablePass pass = 0; pass < AT_NUM_PASSES; pass++)
	{
		...
		foreach(ltab, *wqueue)
		{
			...
			// elog(WARNING, "my log in ATRewriteCatalogs next ATExecCmd");
		  ...
		  AlterTableUtilityContext *context)
{
	...
	switch (cmd->subtype)
	{
		...
		case AT_SetStorage:		/* ALTER COLUMN SET STORAGE */
			...
			break;
		case AT_SetCompression: /* ALTER COLUMN SET COMPRESSION */
			// elog(WARNING, "my log in ATExecCmd next ATExecSetCompression");
...
ATRewriteTable(AlteredTableInfo *tab, Oid OIDNewHeap, LOCKMODE lockmode)
{
	...
	if (newrel || needscan)
	{
		...
		while (table_scan_getnextslot(scan, ForwardScanDirection, oldslot))
		{
			...
			{
				bool		insertIndexes;
				elog(LOG, "my log in ATRewriteTable next table_tuple_insert");
				...
				AlterTableUtilityContext *context)
{
	 ...
	 */
	// elog(WARNING, "my log in ATExecAddColumn next BuildDescForRelation");
					 ...
					 LOCKMODE lockmode)
{
	 ...
	 */
	// elog(WARNING, "my log in ATExecSetCompression next GetAttributeCompression");
...
GetAttributeCompression(Oid atttypid, const char *compression)
{
	...
	// elog(WARNING, "my log GetAttributeCompression 1");
	/*
	...
	// elog(WARNING, "my log GetAttributeCompression 3");
