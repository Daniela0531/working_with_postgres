...
/*
 * Compress a varlena using Brotli.
 *
...
brotli_compress_datum(const struct varlena *value)
{
    ...
    // elog(WARNING, "brotli compressing [value: %p, len: %u]", value, valsize);

    // elog(WARNING, "\ngo to brotli_compress=pglz_compress\n");
...
brotli_decompress_datum(const struct varlena *value)
{
    ...
    elog(LOG, "my log before brotli_decompress=pglz_decompress\n");
...
{
    elog(WARNING, "\ngo to lz4_decompress_datum_slice\n");
#ifndef USE_LZ4
    NO_LZ4_SUPPORT();
    return NULL;				/* keep compiler quiet */
#else
...
CompressionNameToMethod(const char *compression)
{
    if (strcmp(compression, "pglz") == 0) {
        // elog(WARNING, "set for table pglz compression");
        return TOAST_PGLZ_COMPRESSION;
    }
    else if (strcmp(compression, "brotli") == 0) {
        // elog(WARNING, "set for table brotli compression");
