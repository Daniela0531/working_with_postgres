...
{
	// elog(WARNING, "my log heap_prepare_insert next mey be heap_toast_insert_or_update");
	/*
				  ...
				  bool *insert_indexes)
{
	...
	for (i = 0; i < ntuples; i++)
	{
		...
		elog(LOG, "my log in heap_multi_insert next heap_prepare_insert\n");
...
{
	elog(LOG, "my log simple_heap_insert next heap_insert\n");
...
l2:
	...
	if (need_toast || newtupsize > pagefree)
	{
		...
		if (need_toast)
		{
			elog(LOG, "\nin heap_update next heap_toast_insert_or_update\n");
