					...
					int options, BulkInsertState bistate, bool *insert_indexes)
{
	...
	elog(LOG, "my log in heapam_tuple_insert next heap_insert");
								...
								BulkInsertState bistate, uint32 specToken)
{
	...
	/* Perform the insertion, and copy the resulting ItemPointer */
	elog(LOG, "my log in heapam_tuple_insert_speculative next heap_insert");
	heap_insert(relation, tuple, cid, options, bistate);
	ItemPointerCopy(&tuple->t_self, &slot->tts_tid);
							  ...
							  TableScanDesc scan)
{
	 ...
	 */
	elog(LOG, "my log in heapam_index_build_range_scan next heap_getnext");
						   ...
						   ValidateIndexState *state)
{
	 ...
	 */
	elog(LOG, "my log in heapam_index_validate_scan next heap_getnext");
	while ((heapTuple = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
